...
extern "C" void sfwUpdate(float elapsed)
{
	app->mainLoop(1.0f / elapsed);
}
...
bool Application::init( void* device )
{	
	// Initialize engine
	if( !h3dInit(device) )
	{	
...
{
	for(unsigned int i=0; i<sfwKey::Count; ++i)
		 _prevKeys[i] = _keys[i];
	...
	while (sfwGetEvent(&e))
	{
		switch(e.type)
		{
		case sfwEventType::Quit:
			release();
			return;
		case sfwEventType::PointerDown:
			_pointerDown = true;
			_pointerX = e.pointer.x; _pointerY = e.pointer.y;
		break;
		case sfwEventType::PointerUp:
		case sfwEventType::PointerCancelled:
			_pointerDown = false;
		break;
		case sfwEventType::PointerMoved:
			if (_pointerDown)
			{
				mouseMoveEvent( e.pointer.x - _pointerX, e.pointer.y - _pointerY);
				_pointerX = e.pointer.x; _pointerY = e.pointer.y;
			}
		break;
		case sfwEventType::KeyPressed:
			_keys[e.key] = true;
		break;
		case sfwEventType::KeyReleased:
			_keys[e.key] = false;
		break;
		}
	}
...
void Application::keyStateHandler()
{
	...
	if( _keys[sfwKey::F3] && !_prevKeys[sfwKey::F3] )
	{
	...
	if( _keys[sfwKey::F7] && !_prevKeys[sfwKey::F7] )
		_debugViewMode = !_debugViewMode;

	if( _keys[sfwKey::F8] && !_prevKeys[sfwKey::F8] )
		_wireframeMode = !_wireframeMode;
	
	if( _keys[sfwKey::F6] && !_prevKeys[sfwKey::F6] )
	{
		_statMode += 1;
